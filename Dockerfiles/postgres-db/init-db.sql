...
-- Create database
CREATE DATABASE performance_testing;

-- Create user with password (PostgreSQL 15+ syntax)
CREATE ROLE performance WITH LOGIN PASSWORD 'testing';

-- Grant all privileges on database
GRANT ALL PRIVILEGES ON DATABASE performance_testing TO performance;

-- Connect to the new database
\c performance_testing

-- Grant schema privileges (essential for object creation)
GRANT USAGE, CREATE ON SCHEMA public TO performance;

-- Grant table privileges for existing objects
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO performance;

-- Set default privileges for future objects
ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT ALL PRIVILEGES ON TABLES TO performance;
